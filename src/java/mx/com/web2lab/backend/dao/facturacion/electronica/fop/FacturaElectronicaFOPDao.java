...
public class FacturaElectronicaFOPDao {
	
	private static Log iObjLog = LogFactory.getLog(FacturaElectronicaFOPDao.class);	

	
	public String createFactura(FacturaElectronicaBean objFactura,boolean btipounidad) throws Exception{
			...
			for(int inti = 0;inti < objFactura.sizeBodys();inti++) {		
				...
				if (objBody.getIntCantidad() == 0) {
					...
					strRows +=  "<fo:table-row>																				\n" +     			
								...
								"        </fo:table-cell>     																\n" ;
								if(btipounidad){
									strRows += "        <fo:table-cell padding='1pt'>      												\n" +
											   "        	<fo:block text-align='center' font-family='sans-serif' font-size='7pt'>			\n" + 
											   					"ESTUDIOS" 																	+ 
											   "			</fo:block>    																	\n" + 										
											   "        </fo:table-cell>     																\n" ;				
									...
									strRows +="        <fo:table-cell padding='1pt'>														\n" +
											  ...
											  "        <fo:table-cell padding='1pt'>														\n" +
											  "        	<fo:block text-align='right' font-family='sans-serif' font-size='7pt'>			\n" +
											  "	$"	+		new DecimalFormat("#00.00").format(objBody.getDblValorUnitario()) 													+ 
											  "			</fo:block>    																	\n" + 										
											  "        </fo:table-cell>     																\n" +	
											  "        <fo:table-cell padding='1pt'>      												\n" +	
											  "        	<fo:block text-align='right' font-family='sans-serif' font-size='7pt'>			\n" + 
											  "	$"	+		new DecimalFormat("#00.00").format(objBody.getDblImporte())															+ 
											  "			</fo:block>    																	\n" + 		
											  "        </fo:table-cell>     																\n" +
											  "</fo:table-row>     																		\n";
				}
		        ...
		        if (objFactura.getCmarca() == 1) {
		        	...
		        	strDebemos = "DEBO(EMOS) Y PAGARE(MOS) INCODICIONALMENTE A LA ORDEN DE LABORATORIO MOREIRA EN DONDE SE ME REQUIERA A LA VISTA LA CANTIDAD DE $";
		        } else if (objFactura.getCmarca() == 22){
		        	strLogoFactura = "PolabSmall.jpg";
		        	strRFC = "RFCTPolabFactura.jpg";
		        	strDebemos = "DEBO(EMOS) Y PAGARE(MOS) INCODICIONALMENTE A LA ORDEN DE LABORATORIO POLAB EN DONDE SE ME REQUIERA A LA VISTA LA CANTIDAD DE $";
		        } else if (objFactura.getCmarca() == 25){
		        	strLogoFactura = "BiomedicaReferenciaSmall.jpg";
		        	strRFC = "RFCTBiomedicaReferenciaFactura.jpg";
		        	strDebemos = "DEBO(EMOS) Y PAGARE(MOS) INCODICIONALMENTE A LA ORDEN DE LABORATORIO BIOMEDICA DE REFERENCIA EN DONDE SE ME REQUIERA A LA VISTA LA CANTIDAD DE $";
		        } else if (objFactura.getCmarca() == 26){
...
//				            "                                   Descuento" + 
				            ...
				            "                                   -- " + 				            
//				            "                                   $ " + new DecimalFormat("#00.00").format(objFactura.getMsubtotalsuma())  + " " + 
				            "                               </fo:block> " + 
				            ...
				            "                               <fo:block  text-align='right' font-family='Times' font-size='6pt'> " + 
				            "                                   -- " + 				            
//				            "                                   $ " + new DecimalFormat("#00.00").format(objFactura.getMdescuento()) + " " + 
				            "                               </fo:block> " + 
				            "                               <fo:block text-align='right' font-size='3pt' space-before.optimum='5pt' white-space-collapse='false' linefeed-treatment='preserve' white-space-treatment='preserve' wrap-option='no-wrap'> " + 
				            "                               </fo:block> " +
				            "                               <fo:block text-align='right' font-family='Times' font-size='6pt'> " + 
				            "                                   $ " + new DecimalFormat("#00.00").format(objFactura.getMsubtotal()) + " " + 
				            "                               </fo:block> " + "COPAGO" + 
				            "                               <fo:block text-align='right' font-size='3pt' space-before.optimum='5pt' white-space-collapse='false' linefeed-treatment='preserve' white-space-treatment='preserve' wrap-option='no-wrap'> " + 
				            "                               </fo:block> " +
				            "                               <fo:block text-align='right' font-family='Times' font-size='6pt'> " + 
				            "                                   $ " + new DecimalFormat("#00.00").format(objFactura.getMiva()) + " " + 
				            "                               </fo:block> " + 
				            "                               <fo:block text-align='right' font-size='3pt' space-before.optimum='5pt' white-space-collapse='false' linefeed-treatment='preserve' white-space-treatment='preserve' wrap-option='no-wrap'> " + 
				            "                               </fo:block> " + 
				            "                               <fo:block text-align='right' font-family='Times' font-size='6pt'> " + 
				            "                                   $ " + new DecimalFormat("#00.00").format(objFactura.getMtotal()) + " " + 
				            "                               </fo:block> " + 
				            "            </fo:block-container> " + 
