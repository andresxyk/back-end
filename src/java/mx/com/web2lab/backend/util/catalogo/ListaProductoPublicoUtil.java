...
public class ListaProductoPublicoUtil {

	private static Log iObjLog = LogFactory.getLog(ListaProductoPublicoUtil.class);
			
	public int size() {
		return CatalogosBean.lstExamenes.size();
	}
	
	public ListaProductoPublicoUtil() throws Exception {
		iObjLog.debug("Entrando ListaProductoPublicoUtil.LoadExamenes");
		try {
			if (CatalogosBean.lstExamenes.size() < 1) {
				...
				iObjLog.debug("Cargando ListaProductoPublicoUtil.LoadExamenes Examenes " + CatalogosBean.lstExamenes.size() + " Productos " + CatalogosBean.lstProductos.size());
			}		
			iObjLog.debug("Saliendo ListaProductoPublicoUtil.LoadExamenes...." + CatalogosBean.lstExamenes.size());
		} catch (Exception exp) {
			iObjLog.error("Consulta ListaProductoPublicoUtil.LoadExamenes(int kCodigoPostal).....Exception ",exp);				
			throw exp;			
		}
	}
		
	public  List getExamenes(OrdenExamenBean objExamenBean, int intSucursal) throws Exception {
		iObjLog.debug("Entrando ListaProductoPublicoUtil.getExamenes");
		List lstResult = new ArrayList();
		Query objQuery = new Query();
		QueryResults objQueryResult = null;				
		boolean bolExamenes = true;
		try {
			String strSQL = "SELECT * FROM mx.com.web2lab.backend.beans.ap.OrdenExamenBean WHERE  ";
			String strWhere = "";
			if (objExamenBean.getSexamen().trim() != "" && objExamenBean.getSexamen().trim().length() > 1) {
				iObjLog.debug("Entrando ListaProductoPublicoUtil.getExamenes" + objExamenBean.getSexamen().substring(0, 2));				
				if (objExamenBean.getSexamen().substring(0, 2).equals("P.")) {
					strWhere += " sperfil like '%" + objExamenBean.getSexamen().substring(2).trim() + "%' and cperfil>-1 ";
					bolExamenes = false;
				} else {
					strWhere += " sexamen like '%" + objExamenBean.getSexamen() + "%' and cperfil=-1 ";
					bolExamenes = true;
				}
			}
			if (objExamenBean.getStipocomercial().trim() != "" && objExamenBean.getStipocomercial().trim().length() > 1) {
				strWhere += " stipocomercial like '" + objExamenBean.getStipocomercial() + "%'";
			}
			iObjLog.debug("Consulta ListaProductoPublicoUtil.getExamenes...." + strSQL + strWhere);
			if (strWhere.trim().length() > 0) {
				...
				if (lstResult != null) {
					iObjLog.debug("Consulta ListaProductoPublicoUtil.getExamenes....." + lstResult.size());
				} else {
					iObjLog.debug("Consulta ListaProductoPublicoUtil.getExamenes.....VACIO");				
				}				
				iObjLog.debug("Saliendo ListaProductoPublicoUtil.getExamenes");
			}
			return lstResult;
		} catch (QueryParseException qpe) {
			iObjLog.error("Consulta ListaProductoPublicoUtil.getExamenes.....QueryParseException ",qpe);				
			throw qpe;
		} catch (QueryExecutionException qee) {
			iObjLog.error("Consulta ListaProductoPublicoUtil.getExamenes.....QueryExecutionException ",qee);				
			throw qee;
		} catch (Exception exp) {
			iObjLog.error("Consulta ListaProductoPublicoUtil.getExamenes.....Exception ",exp);				
			throw exp;
		} finally {
			objQuery = null;
			objQueryResult = null;
	...
	public  List cotizarProductos(String strQuery, int intSucursal) throws Exception {
		iObjLog.debug("Entrando ListaProductoPublicoUtil.getExamenes");
		Query objQuery = new Query();
		QueryResults objQueryResult = null;
		List lstResult = new ArrayList();
		try {
			String strSQL;
			strSQL = "SELECT * FROM mx.com.web2lab.backend.beans.ap.OrdenExamenBean WHERE (cexamen in (" + strQuery + ") and cperfil=-1) or (cperfil in (" + strQuery + "))";
			iObjLog.debug("Consulta ListaProductoPublicoUtil.getExamenes...." + strSQL);
			iObjLog.debug("Consulta ListaProductoPublicoUtil.getExamenes....Productos " + CatalogosBean.lstProductos.size());
	...
	public  List cotizarProductosConvenio(String strQuery, int intSucursal,int intConvenio) throws Exception {
		...
		try {
			...
			if (lstReturn != null) {
				iObjLog.debug("Consulta ListaProductoPublicoUtil.getExamenes....." + lstReturn.size());
			} else {
				iObjLog.debug("Consulta ListaProductoPublicoUtil.getExamenes.....VACIO");				
			}				
			iObjLog.debug("Saliendo ListaProductoPublicoUtil.getExamenes");
	...
	private List loadExamenes(List lstLoad) throws Exception {
		...
		try {			
			if (lstLoad != null) {
				if (lstLoad.size()>0) {
					for(int inti=0;inti<lstLoad.size();inti++){
						...
						OrdenExamenBean objExamenCatalogo = (OrdenExamenBean)lstLoad.get(inti);
			...
			iObjLog.error("Consulta ListaProductoPublicoUtil.loadExamenes..... ",exp);				
			throw exp;			
		} finally {
			lstLoad.clear();
			lstLoad = null;
